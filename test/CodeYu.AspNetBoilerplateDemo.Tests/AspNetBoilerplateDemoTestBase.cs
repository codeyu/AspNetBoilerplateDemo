...
{
    public abstract class AspNetBoilerplateDemoTestBase : AbpIntegratedTestBase<AspNetBoilerplateDemoTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected AspNetBoilerplateDemoTestBase()
        {
        ...
        protected void UsingDbContext(Action<AspNetBoilerplateDemoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AspNetBoilerplateDemoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AspNetBoilerplateDemoDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AspNetBoilerplateDemoDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AspNetBoilerplateDemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AspNetBoilerplateDemoDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AspNetBoilerplateDemoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AspNetBoilerplateDemoDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AspNetBoilerplateDemoDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AspNetBoilerplateDemoDbContext, Task<T>> func)
        {
            T result;
